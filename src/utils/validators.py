...
class InputValidator:
    ...
    def validate_date(date_str: str) -> Tuple[bool, Optional[str]]:
        ...
        if date_str > max_future_date:
            return False, "예약 가능한 기간을 초과했습니다. (최대 1년 이내)"
    ...
    def validate_station_name(station: str) -> Tuple[bool, Optional[str]]:
        ...
        # Check maximum length (allow names with parentheses e.g. 울산(통도사))
        if len(station) > 15:
            return False, "역 이름이 너무 깁니다. 올바른 역 이름을 입력해주세요."

        # Allow Korean/alphanumeric, spaces, hyphens, and parentheses
        allowed_extra = set(' -()')
        if not all(c.isalnum() or c in allowed_extra for c in station):
            return False, "역 이름에 특수문자를 사용할 수 없습니다."

        # Check against actual station database (exact or unique alias)
    ...
    def validate_train_type_choice(choice: str) -> Tuple[bool, Optional[str]]:
        ...
        """
        if not choice:
            return False, "열차 종류를 선택해주세요. (1: Flagship만, 2: Entry / 전체)"
        ...
        if choice not in ["1", "2"]:
            return False, "1 또는 2를 입력해주세요. (1: Flagship만, 2: Entry / 전체)"
