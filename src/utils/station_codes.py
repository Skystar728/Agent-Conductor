...
KORAIL_STATION_DB_URL = settings.TRAIN_STATION_LIST_URL

# Redis cache key and TTL
REDIS_STATION_CACHE_KEY = "train:station_list"
REDIS_STATION_CACHE_KEY_LEGACY = "korail:station_list"
...
class StationManager:
    ...
    def _get_from_redis(self) -> Optional[Set[str]]:
        ...
        try:
            ...
            if not cached_data:
                cached_data = self._redis_client.get(REDIS_STATION_CACHE_KEY_LEGACY)
            if cached_data:
...
def _station_base_name(station_name: str) -> str:
    """Return station name without parenthetical suffix (e.g. 울산(통도사) -> 울산)."""
    return station_name.split('(', 1)[0]


def resolve_station_name(station_name: str) -> Optional[str]:
    """
    Resolve user input to a canonical Korail station name.

    Resolution order:
    1. Exact match (e.g. 서울, 울산(통도사))
    2. Unique base-name match (e.g. 울산 -> 울산(통도사), 진부 -> 진부(오대산))
       Ambiguous bases like 판교 (판교(경기)/판교(충남)) return None.

    Args:
        station_name: User-entered station name (without '역')

    Returns:
        Canonical station name, or None if not uniquely resolvable
    """
    if not station_name:
        return None
    ...
    base_matches = [
        ...
        if _station_base_name(s) == station_name
    ]
...
def is_valid_station(station_name: str) -> bool:
    """
    Check if station name is valid (exact or uniquely resolvable alias).
    ...
    """
    return resolve_station_name(station_name) is not None
...
def get_similar_stations(station_name: str, max_results: int = 5) -> list:
    ...
    # Already uniquely resolvable — no suggestions needed
    if resolve_station_name(station_name) is not None:
        return []

    matches = []

    # 1. Same base name (e.g. 판교 -> 판교(경기), 판교(충남))
    for valid_station in valid_stations:
        if _station_base_name(valid_station) == station_name:
