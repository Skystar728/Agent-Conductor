...
class MultiReservationReminderService:
    ...
    _reminder_threads = {}  # chat_id -> threading.Thread (shared across instances)

    def __init__(self, storage: StorageInterface, telegram: TelegramService):
        """Initialize the reminder service."""
        self.storage = storage
        self.telegram = telegram
        self.reminder_threads = MultiReservationReminderService._reminder_threads
    ...
    def _generate_reminder_message(self, status: MultiReservationStatus) -> str:
        ...
        if pending_count > 0:
            ...
            lines.append(f"📱 스마트폰: '코레일톡' 앱 ➡️ 하단 [승차권 확인/장바구니]")
            lines.append(f"🌐 웹 결제: {settings.KORAIL_PAYMENT_URL}")
