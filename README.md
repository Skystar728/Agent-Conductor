...
- **비밀번호 즉시 파기 및 보안 강화**: 입력된 계정 비밀번호는 텔레그램 채팅창에서 즉시 삭제되며, 탐색 중 암호화되어 임시 유지된 후 결제 완료, 취소(`/cancel`), 또는 탐색 종료 즉시 영구 파기되어 저장소에 일절 보존되지 않습니다.
- **부재중 결제 기한 초과 시 자동 재시작**: 10분 결제 리마인더 기한이 만료되었을 때 실제 발권 여부를 자동 확인하여, 미결제(부재중)인 경우 미결제 예약을 정리하고 자동으로 여정 탐색을 재시작(최대 3회)
- **적응형 트래픽 보호 및 응답 무결성**: 요청 간격 무작위 지터, 429 쿨다운, 차단 감지 서킷 브레이커, 정기 점검 모드 감지 및 비표준 인코딩(모지바케) 자동 UTF-8 복구 훅 내장
...
| 명령어 | 설명 |
|---|---|
| `/start` | 새로운 여정 조율 시작 (보안을 위해 계정 정보는 상시 재입력 요구) |
| `/status` | 현재 사용자의 여정 탐색 상태, 계정, 조건 실시간 확인 (메신저 공식 메뉴 등록) |
| `/cancel` | 진행 중인 대화 및 백그라운드 탐색 취소 (세션 및 인증 정보 즉시 파기) |
