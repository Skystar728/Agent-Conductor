...
class TestStationNameValidation:
    ...
    def test_valid_station_ulsan_alias(self):
        """Test short name '울산' resolves to 울산(통도사)."""
        from utils.station_codes import resolve_station_name

        valid, _ = InputValidator.validate_station_name("울산")
        assert valid is True
        assert resolve_station_name("울산") == "울산(통도사)"

    def test_valid_station_ulsan_full_name(self):
        """Test full parenthetical station name."""
        from utils.station_codes import resolve_station_name

        valid, _ = InputValidator.validate_station_name("울산(통도사)")
        assert valid is True
        assert resolve_station_name("울산(통도사)") == "울산(통도사)"

    def test_ambiguous_station_pangyo(self):
        """Test ambiguous base name '판교' is rejected with suggestions."""
        valid, error = InputValidator.validate_station_name("판교")
        assert valid is False
        assert "판교" in error
    ...
    def test_invalid_station_too_long(self):
        ...
        valid, error = InputValidator.validate_station_name("가나다라마바사아자차카타파하")
        assert valid is False
