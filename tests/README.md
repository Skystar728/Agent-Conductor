...
This test suite provides comprehensive coverage for the **Agent-Conductor** application. The tests are organized into three main categories:

- **Unit Tests**: Test individual functions, validators, encryption, NLP parsers, and in-flight modifications (138 tests)
- **Integration Tests**: Test service interactions, conversation state machine, Redis storage, and webhooks
...
│   ├── test_natural_language_parser.py    # NLP slot extraction & heuristics
│   ├── test_edit_options.py               # In-flight natural language & /edit
│   ├── test_crypto.py                     # AES-128/Fernet encryption & backward compatibility
│   ├── test_ai_agent_service.py           # AI error analysis & HITL policies
│   ├── test_auto_restart_on_timeout.py    # Autonomous retry & timeouts
│   ├── test_duplicate_notifications.py    # Notification deduplication
│   ├── test_inline_buttons.py             # Keyboards and inline flows
│   ├── test_korail_pacing.py              # Traffic pacing & jitter tests
│   ├── test_menu_and_restart.py           # Session restart & menus
│   ├── test_qa_fixes.py                   # Regressions & security checks
│   └── test_webhook_security.py           # Sensitive input masking & cleanup
├── integration/                           # Integration tests
│   ├── test_conversation_handler.py      # Conversation flow tests (19 tests)
│   ├── test_webhook.py                   # Webhook handling tests (18 tests)
│   ├── test_payment_reminder.py          # Payment reminder tests
│   ├── test_refactored_app.py            # Architecture tests
│   └── test_reservation_service.py       # Reservation management tests
├── e2e/                                   # End-to-end tests
│   └── test_full_reservation_flow.py     # Complete flows
...
All dependencies and test utilities are pre-installed within the Docker container:
- `pytest`
- `pytest-mock` (mocking utilities)
- `freezegun` (time manipulation)
- `redis`

Services can be started via:
```bash
docker compose up -d
```
...
```bash
# Validator tests
docker compose exec app pytest tests/unit/test_validators.py -v

# Conversation handler tests
docker compose exec app pytest tests/integration/test_conversation_handler.py -v

# Full flow tests
docker compose exec app pytest tests/e2e/test_full_reservation_flow.py -v
```
...
```bash
# Run single test
docker compose exec app pytest tests/unit/test_validators.py::TestPhoneNumberValidation::test_valid_phone_with_hyphens -v

# Run all tests in a class
docker compose exec app pytest tests/integration/test_conversation_handler.py::TestConversationHandler -v
```
...
docker compose exec app pytest tests/ -k "payment" -v

# Stop on first failure
docker compose exec app pytest tests/ -x

# Show print statements
docker compose exec app pytest tests/ -s

# Parallel execution (if installed pytest-xdist)
docker compose exec app pytest tests/ -n auto
```
